...
- 文件长度保持克制，但不设硬性行数上限。长度是"停下来审视"的触发器，不是"必须服从"的规则：当文件明显过长时，先找**自然接缝**（一个可独立命名的职责能干净分离的点），有真实接缝才拆，没有就保持完整并留一行注释说明原因。**禁止为凑行数而硬拆分**——人为拆分比一个诚实的大文件更糟
- 单个函数保持简短、聚焦单一职责，不设硬性行数上限。函数最大可接受长度与其复杂度成反比：概念简单的线性流程可以稍长，复杂/高嵌套函数必须更短。过长时优先判断是否承担了过多职责，按职责拆分
- 文件/组件的**放置位置由"使用范围"决定，不由"行数"决定**（参考 Next.js colocation 模式）：
  - 单路由专用 → 放在该路由文件夹内（如 `app/blog/_components/`），无论多长
  - 同父路由下多个路由共用 → 上移到共同父级
  - 跨多个不相关路由的**领域模块**（可独立删除/迁移） → 提升到 `src/features/<domain>/`
  - 无业务逻辑的纯展示组件 → `src/components/ui/`
  - 通用工具/hooks → `src/lib/`
  - **`src/features/` 是"领域聚合层"，不是"长文件回收站"**——只有当一个领域跨路由复用时才提升，单路由用的代码即使变长也留在路由内
