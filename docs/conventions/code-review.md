...
- 检查阅读页 `'use client'` 边界是否串进完整 Canonical IR、`originalSource`、逐节点 `sourceText` 或全站 asset manifest
- 检查文章是否只编译一次，article profile 的 `DocumentRenderer` 是否消费已编译文档
- 检查路由 `loading.tsx` 的书册是否会在目标页完成前自行卸载
- 检查文章链接是否禁止视口批量预取，预取是否只发生在展开组/悬停/聚焦
...
- 检查是否超出 [规格 13.2 性能预算](../specs/blog-content-engine.md#132-性能预算)；阶段验收是否实测并记录了首屏体积，且包含最大正式文章的 RSC / HTML
