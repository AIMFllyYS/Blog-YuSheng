...
| **9982** | 静态产物预览（验证 `out/` 产物） | `pnpm preview` |
| **9983** | 首页电影化分支的隔离验收预览（避免占用其他工作树的 9981） | `node node_modules/next/dist/bin/next dev -p 9983` |
| 9984-9989 | 预留（未来本地工具 / mock 服务按序取用，取用后登记到本表） | — |

规则：
- 本项目所有本地服务固定使用 998x 段，避免与其他项目冲突
- 新增本地服务必须先查本表再取端口，取用后更新本表
- `pnpm preview` 只是静态文件服务器，**不读 `edgeone.json`**：响应头、redirects、rewrites 在本地一律不生效，相关断言只能在部署后对公网 URL 做
