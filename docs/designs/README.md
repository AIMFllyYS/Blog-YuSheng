...
- [blog-reader-prototype.html](./blog-reader-prototype.html) — `/blog/` 与 `/blog/<slug>/` 的 **1:1 视觉与交互对标**（浏览器直接打开，零依赖，不接入任何真实功能）。实现这两页时以本文件为准；绳挂导航、通知、弹窗、动效也以本文件为全站模板，不得另起一套外观。
