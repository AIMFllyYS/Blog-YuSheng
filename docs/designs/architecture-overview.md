...
- **客户端投影是独立架构边界。** 构建期 Canonical IR 可以胖，阅读 RSC / HTML 必须瘦。完整 `CompiledDocument`、`originalSource`、逐节点 `sourceText` 和全站资产表不得进入阅读首屏的 `'use client'` 组件。导出源码与划词索引用静态 sidecar 按需加载。同一篇文章只编译一次。
...
- 进页书册遮罩分两种语义：直达仪式可以定时揭开已在 HTML 中的正文；客户端路由 loading 必须保持到目标页替换它，不能先变成空页。
- 文章预取按意图，不按「链接进了 DOM」。折叠的书库不得批量预取全部正文。
- 具体预算数字与实测要求见功能规格 13.1–13.3；每阶段验收必须实测并记录首屏体积，且必须包含体积最大的正式文章的 RSC / HTML。
