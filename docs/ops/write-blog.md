...
标题建议从 `#` 或 `##` 写起，和 `title` 对齐即可。正式文章的第一个 `#` 必须与 frontmatter 的 `title` 完全一致；如果需要保留旧标题或口头副标题，用正文引用块（`>`）表达，不新增 `subtitle` 字段。`description` 仍是列表 / OG 摘要，不能拿来伪装副标题。

### 2.2.1 正文正本与导入素材保真

`content/posts/<slug>/index.md` 是作者正文的唯一正本。AI 可以帮助检查错别字、明显病句、标点和排版，但不得把作者文字重新概括、扩写、改成第三人称、替换价值判断，或为了“更像文章”删除事实、例子、情绪和有意重复。

从录音、TXT、PDF、DOCX 或 HTML 导入时，只允许做轻度保真整理：去除明确的语音转写乱码、说话人标签、时间戳、同一次口吃造成的无意义重复，删除素材末尾的 `以上内容由AI生成，仅供参考`，并在不改变措辞和含义的前提下重新分段。HTML 仍走安全的 `html-embed` 资源，PDF / DOCX 原件留在只读来源，不复制进 Git；已有电话、学号等隐私删改不得在“恢复原文”时重新引入。

每篇导入文都应保留 1–2 个可从原始来源直接核对的正文锚点。若要进行超出上述范围的实质改写，必须先更新来源说明和锚点清单，不能只修改测试字符串。
